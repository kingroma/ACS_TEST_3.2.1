...
/**
 * node xmpp-msg.js -n 1 -t 1 -i 1 -m 1 -f 3 -r
 * 185.3.160.117
 * --ip 185.3.160.117 --stbId  987654321 --messageId 123456
 * 
 * --ip 185.3.160.117 --ip2 185.3.160.117 --stbId  987654321 --messageId 123456 --ssl TLS
 * 
 * --ip 185.3.160.117 --ip2 185.3.160.117 --stbId  987654321 --messageId ABCD- --ssl TLS --loop 3
 * */
public class Main {
			...
			if (args.length%2 != 0 ) {
				...
				for ( int i = 0 ; i < args.length ; i = i + 2) {
					...
					case "--offset":
						int offsetTemp = 1000 ; 
						try {
							offsetTemp = Integer.parseInt(arg2);
						} catch (Exception e) {
	...
	public static void sendIq() {
		try {
			...
			gconfig.setSecurityMode(SecurityMode.required);
			
			
			// if ( ssl != null && !ssl.isEmpty() )
			//	gconfig.setCustomSSLContext(sslContext);
			
			XMPPConnection conn = new XMPPConnection(gconfig);
			conn.connect();
...
//				}
			...
			conn.addPacketListener(new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					System.out.println("receive : " + packet.toXML());
				}
			}, new PacketFilter() {
